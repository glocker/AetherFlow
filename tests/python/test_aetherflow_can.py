...
from bridge_service.aetherflow_can import (
...
def test_can_id_helpers() -> None:
    assert make_can_id(AetherflowCanFrameClass.SYNC, 0) == 0x080
    assert make_can_id(AetherflowCanFrameClass.HEARTBEAT, 1) == 0x701
    assert make_can_id(AetherflowCanFrameClass.REQUEST, 1) == 0x601
    assert make_can_id(AetherflowCanFrameClass.REPLY, 1) == 0x581

    parsed = parse_can_id(0x581)
    assert parsed.frame_class == AetherflowCanFrameClass.REPLY
...
def test_single_frame_roundtrip() -> None:
    ...
    frames = fragment_packet(AetherflowCanFrameClass.REQUEST, 1, packet)
    assert len(frames) == 1
    ...
    assert status == AetherflowCanStatus.OK
...
def test_multi_frame_roundtrip() -> None:
    ...
    packet = packet_build(3, 25, payload)
    frames = fragment_packet(AetherflowCanFrameClass.REPLY, 1, packet)
    ...
    for index, frame in enumerate(frames):
        ...
        expected = AetherflowCanStatus.OK if index + 1 == len(frames) else AetherflowCanStatus.ERR_IN_PROGRESS
...
def test_sequence_error_resets_reassembly() -> None:
    ...
    state = AetherflowCanReassembly()
    assert reassembly_accept(state, frames[0])[0] == AetherflowCanStatus.ERR_IN_PROGRESS
    assert reassembly_accept(state, corrupted)[0] == AetherflowCanStatus.ERR_SEQUENCE
