...
"""Validate AetherFlow CAN Protocol compatibility vectors.

Default mode validates committed golden vectors against a small pure-Python
reference implementation of the current AetherFlow CAN Protocol. This
intentionally has no third-party dependency, so compatibility checks are
...
external protocol package is importable in the current environment.
"""
...
def validate_vector(vector: Mapping[str, object]) -> list[str]:
    ...
    # Golden → Python path parses committed bytes and checks semantic fields
    try:
        ...
        errors.append(f"{name}: golden vector parse failed: {exc}")

    # Python encoder path independently re-encodes bytes and compares exact output
    try:
        encoded_packet = build_packet(service, subtype, payload)
        if encoded_packet != expected_packet:
            errors.append(
                f"{name}: Python packet {encoded_packet.hex()} != golden packet {expected_packet.hex()}"
            )
        ...
        if encoded_frames != frames:
            ...
            errors.append(f"{name}: Python frames {actual} != golden frames {expected}")
    except ValueError as exc:
        errors.append(f"{name}: Python encode failed: {exc}")
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "vectors",
        nargs="?",
        default=Path("compat/vectors/aetherflow_can_vectors.json"),
        type=Path,
        help="Path to committed compatibility vector JSON.",
    )
    parser.add_argument(
        "--backend",
    ...
    if schema != "aetherflow.can_protocol.vectors":
    ...
    print(f"validated {len(vectors)} AetherFlow protocol vector(s)")
    return 0
